...
if %1 == 1 (echo Making BitReKt Windows DEMO VERSION distribution ...) else (echo Making BitReKt Windows FULL VERSION distribution ...)
...
REM - msbuild BitReKt-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild BitReKt.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild BitReKt.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\BitReKt.aaxplugin\Contents\Win32\BitReKt.aaxplugin --out .\build-win\aax\bin\BitReKt.aaxplugin\Contents\Win32\BitReKt.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\BitReKt.aaxplugin\Contents\x64\BitReKt.aaxplugin --out .\build-win\aax\bin\BitReKt.aaxplugin\Contents\x64\BitReKt.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\BitReKt.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\BitReKt.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "BitReKt Installer" ".\installer\BitReKt Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\BitReKt Installer.exe" ".\installer\BitReKt Demo Installer.exe"
REM -del ".\installer\BitReKt Installer.exe"
REM -)
