...
def  main():
  ...
  installer = "\installer\BitReKt Installer.exe"
   
  if demo:
    installer = "\installer\BitReKt Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BitReKt manual.pdf" 
  ]
  ...
  ZIPNAME = "BitReKt-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BitReKt-v" + FULLVERSIONSTR + "-win-demo.zip"
